...
export default function DiseaseDetection() {
  ...
  useEffect(() => {
    try {
      const savedMessages = localStorage.getItem('diseaseDetectionChat');
      if (savedMessages) {
  ...
  useEffect(() => {
    if (messages.length > 0) {
      localStorage.setItem('diseaseDetectionChat', JSON.stringify(messages));
    }
  }, [messages]);
  ...
  const handleCameraCapture = async () => {
    try {
      // Start camera stream with relaxed constraints
      const constraints = {
        video: { 
    ...
    if (videoRef.current && canvasRef.current && cameraActive) {
      try {
        ...
        try {
          ...
          alert("Unable to capture photo. Please try uploading an image instead.");
        }
      }
    }
  };
  ...
  const analyzeImage = async () => {
    ...
    try {
      ...
      console.log(`Using language: ${currentLanguage.name} (Frontend: ${currentLanguage.code}, API: ${apiLanguageCode})`);
      ...
      // Check if it's a valid plant image - the API returns a success response even for non-plant images
      // but the diagnosis content will indicate if it's not a plant
      ...
      if (isNonPlantImage) {
        ...
        // Remove the analyzing message and add error message
            ...
            <div className="p-4 border-b bg-muted/30">
              <div className="flex items-center justify-between">
                ...
                <Button
                  ...
                  onClick={() => {
                    localStorage.removeItem('diseaseDetectionChat');
                    setMessages([welcomeMessage]);
                    toast({
